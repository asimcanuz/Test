...
// Health Check Endpoint (Sağlık Kontrolü) 🏥
...
async fn main() -> std::io::Result<()> {
    ...
    // Rate Limiting Yapılandırması (Rate Limiting Configuration)
    // Her IP için saniyede 2 istek, 10 ani patlama (2 req/s per IP, burst of 10)
    let governor_conf = GovernorConfigBuilder::default()
        .per_second(2)  // Saniyede 2 istek (2 requests per second)
        .burst_size(10) // Ani 10 isteklik patlama (Burst of 10 requests)
        .finish()
        .unwrap();

    println!("🚀 CDN Servisi Başlatıldı (Port: 8080)");
    println!("📂 Depolama Yolu: {}", state.base_storage_path);
    println!("⚡ Yüksek Trafik Özellikleri Aktif:");
    println!("   - Rate Limiting: 2 req/s (burst: 10)");
    println!("   - Sıkıştırma: Aktif (gzip, deflate, br)");
    println!("   - Worker Sayısı: {}", num_cpus::get());
    println!("   - Keep-Alive: 75 saniye");

    HttpServer::new(move || {
        App::new()
            // Sıkıştırma Middleware (Compression) - gzip, deflate, brotli
    ...
    .workers(num_cpus::get()) // CPU sayısı kadar worker (Workers equal to CPU count)
    .keep_alive(Duration::from_secs(75)) // Keep-alive süresini ayarla (Set keep-alive duration)
